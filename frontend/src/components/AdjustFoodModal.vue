...
function saveVariant() {
  const overrides = { ...(sourceEntry?.overrides || {}) }
    ...
    <div class="adjust-food-content">
      ...
      <div class="adjust-food-list">
        <div v-for="row in rows" :key="row.ingredientId" class="adjust-food-row">
          ...
          <input v-model.number="row.amount" class="adjust-food-qty" type="number" min="0" step="any" :aria-label="`${getIngredient(row.ingredientId)?.name || 'Ingredient'} amount`" />
          <span class="adjust-food-unit">{{ getIngredient(row.ingredientId)?.unit === 'g' ? 'g' : 'each' }}</span>
        </div>
      </div>
      <button class="btn btn-primary primary-wide" type="button" @click="saveVariant">Save adjustment</button>
    </div>
  </BaseModal>
</template>
